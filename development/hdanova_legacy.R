...
hdanova2 <- function(formula, data, subset, weights, na.action, family,
  ...
  if(any(grepl("|", formula, fixed = TRUE)) || any(grepl("r(", formula, fixed = TRUE)))
    stop("hdanova2() Phase 1 supports fixed-effect models only. Use hdanova() for mixed models.")
  if(!identical(unrestricted, FALSE))
    stop("'unrestricted' is not supported in hdanova2() Phase 1.")
  if(!identical(add_error, FALSE))
    stop("'add_error' is not supported in hdanova2() Phase 1.")
  if(!identical(aug_error, "denominator"))
    stop("'aug_error' is not supported in hdanova2() Phase 1.")
  if(!identical(use_ED, FALSE))
    stop("'use_ED' is not supported in hdanova2() Phase 1.")
  if(!identical(pca.in, FALSE) && !identical(pca.in, 0))
    stop("'pca.in' is not supported in hdanova2() Phase 1.")
  if(!is.null(REML))
    stop("'REML' is not supported in hdanova2() Phase 1.")
  if(!identical(equal_baseline, FALSE))
    stop("'equal_baseline' is not supported in hdanova2() Phase 1.")
  ...
  # Fit single multivariate LM model and derive model structures.
  weights_vec <- stats::model.weights(mf_data)
  lm_args <- list(formula = formula, data = mf_data)
  if(!is.null(weights_vec))
    lm_args$weights <- weights_vec
  mod <- do.call(lm, lm_args)
  M <- model.matrix(mod)
  assign <- attr(M, "assign")
  ...
  qr_res <- .calc_SS_qr_loop(M, Y, assign, effs, SStype)
  ...
  err_map <- .map_error_terms_mom(df_terms = df_terms,
                                  MS_matrix = MS_matrix,
                                  MS_res = MS_res,
                                  df_res = df_res,
                                  term_labels = effs)
  ...
  F_matrix <- P_matrix <- matrix(NA_real_, nrow = n_terms, ncol = p,
  ...
  if(ncol(coefs) != p)
    coefs <- matrix(coefs, nrow = nrow(coefs), ncol = p)
  colnames(coefs) <- colnames(Y)

  effects <- setNames(lapply(effs, function(eff) modFra[[eff]]), effs)
  LS_aug <- error <- setNames(lapply(effs, function(eff) LS[[eff]] + residuals), effs)
  ...
  dfDenom <- c(stats::setNames(rep(df_res, length(df_terms)), effs), Residuals = 0)
  denoms <- c(rep(which(names(dfDenom) == "Residuals"), length(dfDenom) - 1), NA)
  names(denoms) <- names(dfDenom)
...
.calc_SS_qr_loop <- function(X, Y, assign, term_labels, SStype){
  ...
  for(i in seq_len(n_terms)){
    ...
    SS_matrix[i, ] <- colSums(effects[[term_labels[i]]]^2)
  }
...
.map_error_terms_mom <- function(df_terms, MS_matrix, MS_res, df_res, term_labels){
  n_terms <- length(term_labels)
  p <- ncol(MS_matrix)
